...
`EZbakRArrowData` creates an object of class `EZbakRArrowData` and
...
- metadf:
  ...
  - `tl`: Metabolic label time. There are several edge cases to be aware
    ...
    - If more than one metabolic label was used in the set of samples
      ...
      column in the cBds that the label whose incubation time will be
...
An EZbakRArrowData object. This is simply a list of the provide `cBds`
and `metadf` with class `EZbakRArrowData`
