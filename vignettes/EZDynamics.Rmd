...
# 2) Use the reference_levels and experimental_levels parameters
# 3) Specify the "design_factor" (treatment in this case) and
# parameter (let's compare k2).
ezbdo <- CompareParameters(ezbdo,
                           parameter = "logk2",
                           design_factor = "treatment",
                           reference_levels = "treatment1",
                           experimental_levels = "treatment2")
...
1 is technically ideal, though getting spike-ins to work well can be a easier said than done. If scale factors are not provided, EZbakR will try 3. This strategy works by modeling the total fraction of reads that are labeled in a given sample as a function of the same value in other samples. For example, 
